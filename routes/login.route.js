...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        if (getuser && getuser.password === req.body.pass) {
            console.log("userdash details",getuser.username, getuser, getadmin)
            return res.render("userdash.ejs", { username: getuser.username, getuser });
        }

        if (getadmin && getadmin.password === req.body.pass) {
            return res.render("admindash.ejs",{ username: getuser.username, getuser });
        }
...
router.post("/reset", async (req, res) => {
    ...
    try {
        ...
        const resetLink = `http://localhost:7977/reset-password/${email}`;
